...
from utils.formatters import display_product_cards, display_combo_cards, format_product_summary
...
with tab_chat:
    for msg in get_messages():
        with st.chat_message(msg["role"]):
            ...
            if msg.get("combo") and msg["combo"].get("groups"):
                display_combo_cards(msg["combo"])
            elif msg.get("products"):
...
if user_input:
    ...
    with st.chat_message("assistant"):
        ...
        combo = response.get("combo")
        is_combo = combo and combo.get("groups")

        # For combo results, DON'T add the flat product summary — the grouped cards handle it
        stored_text = response_text
        if products and not is_combo:
        ...
        with st.expander("🔍 Debug: Gemini filters & pipeline", expanded=False):
            st.json(response.get("filters", {}))
            if response.get("combo"):
                st.write("**Combo detected:**", response["combo"].get("groups", []))
            st.caption(f"Products returned: {len(products) if products else 0}")

        # ── Combo display (multi-product budget) ──
        if is_combo:
            n_groups = len(combo["groups"])
            n_total = sum(len(g.get("products", [])) for g in combo["groups"])
            st.markdown(
                f"---\n🎯 **Smart Combo: {n_groups} categories, "
                f"{n_total} products within your ₹{combo.get('total_budget', 0):,.0f} budget:**"
            )
            display_combo_cards(combo)
        elif products:
            st.markdown(f"---\n✨ **Found {len(products)} product(s) matching your style:**")
            display_product_cards(products)

    add_message("user", user_input)
    add_message("assistant", stored_text, products=products, combo=combo if is_combo else None)
    st.rerun()
